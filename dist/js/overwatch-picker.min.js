...
angular.module('overwatch-hero-picker', ['overwatch-hero-picker.config', 'ngDialog', 'ngTouch']);
'use strict';
...
angular.module('overwatch-hero-picker').component('owHeroPicker', {
    template:'<header><div class="logo"><img class="full" src="/dist/media/overwatch_03_flat.svg"> <img class="small" src="/dist/media/ot-white.svg"> <small>Overwatch<sup>TM</sup> is a registered trademark of Blizzard Entertainment</small></div><div class="header-title picker">Pick hero(es) to counter</div><div class="header-title map">Choose a map <span class="modeO">Offense</span>, <span class="modeD">Defense</span> or <span class="modeKOTH">Assault</span></div><div class="header-title recommended">You should pick (from <span class="modeD">best</span> to <span class="modeO">worst</span>)</div><div class="right-tools"><button ng-dialog="how-to-use.html" ng-dialog-class="ngdialog-theme-default">Help</button> <button class="hamburger" ng-click="navOpened = !navOpened">Player settings</button></div><nav class="nav" ng-class="{open : navOpened}"><ow-reset-player-hero-rating heroes-rating="$ctrl.heroesRating" class="pull-right"></ow-reset-player-hero-rating><ow-player-hero-rating heroes-rating="$ctrl.heroesRating"></ow-player-hero-rating></nav></header><main class="content"><ow-hero-list class="opponents" selected-heroes="$ctrl.selectedHeroes"></ow-hero-list><ow-map-list selected-map="$ctrl.selectedMap"></ow-map-list><div class="recommendation" ng-show="$ctrl.heroesList.length"><ul class="recommended clearfix"><li ng-repeat="hero in $ctrl.heroesList | filter: $ctrl.recommendedFilter | limitTo: $ctrl.heroesLimit"><img ng-src="{{ hero.avatar }}"><ow-hero-picker-score final="$ctrl.heroFinalScore[hero.id]" counter="$ctrl.heroCounterScore[hero.id]" map="$ctrl.heroMapScore[hero.id]"></ow-hero-picker-score></li></ul><input type="checkbox" id="hero-limit" ng-click="$ctrl.toggleHeoresLimit($event)"><label for="hero-limit">Show all ratings</label></div></main>',
...
angular.module('overwatch-hero-picker').component('owHero', {
    ...
    template:'<img ng-src="{{$ctrl.hero.avatar}}"><div class="detail-wrapper"><span class="name">{{$ctrl.hero.name}}</span><div ng-transclude></div></div>',
    transclude: true
});
'use strict';
...
angular.module('overwatch-hero-picker').component('owMapList', {
    ...
    template:'<ul class="list"><li ng-repeat="map in $ctrl.maps" class="{{\'mode\' + map.mode}}" ng-class="{ \'selected\': $ctrl.selectedMap === map, \'disabled\' : $ctrl.selectedMap !== null && map !== $ctrl.selectedMap }"><button ng-click="$ctrl.selectMap(map)"><img ng-src="{{map.image}}"><div class="detail-wrapper"><span class="name">{{map.name}}</span></div></button></li></ul><button class="clear" ng-click="$ctrl.selectedMap = null">Reset</button>',
...
angular.module('overwatch-hero-picker').component('owPlayerHeroRating', {
    ...
    template:'<ul class="list"><li ng-repeat="hero in $ctrl.heroes | orderBy: \'name\'"><ow-hero hero="hero"><span class="rating-display">(<span ng-show="$ctrl.heroesRating[hero.id] > 0">+</span>{{$ctrl.heroesRating[hero.id]}})</span> <input type="range" min="-5" max="5" ng-model="$ctrl.heroesRating[hero.id]"></ow-hero></li></ul>',
    controller: function controller(HeroesService) {
        var _this = this;
...
angular.module('overwatch-hero-picker').service('Api', function Api($log, $rootScope, $q, WS_API) {
    var socket = new eio.Socket(WS_API, {
...
angular.module('overwatch-hero-picker').service('HeroPickerService', function HeroPickerService($q, HeroesService) {
    ...
    this.getRecommendedHero = function (opposingHeroes, map, heroesRating) {
        return $q.all({
            ...
            var finalRating = {};
            var nbOpponent = Object.keys(opposingHeroes).length;
            ...
            for (var pickHeroId in result.matching) {
                if (result.matching.hasOwnProperty(pickHeroId)) {
                    ...
                    finalRating[pickHeroId] = recommendedByMap[pickHeroId] * 0.4 + heroesRating[pickHeroId] * 0.6;
...
angular.module('overwatch-hero-picker').service('HeroesService', function HeroesService(Api) {
    this.getHeroes = function () {
        return Api.getTopic('heroes');
    };
    this.getMatching = function () {
        return Api.getTopic('matching');
    };
});
'use strict';
...
angular.module('overwatch-hero-picker').service('MapsService', function MapsService(Api) {
    this.getMaps = function () {
        return Api.getTopic('maps');
    };
});
angular.module("overwatch-hero-picker.config", [])
.constant("WS_API", "ws://api.overwatch-tools.com:7654/");
