...
angular.module('overwatch-hero-picker').component('owHeroList', {
    ...
    template:'<ul class=list><li ng-repeat="hero in $ctrl.heroes | filter: search | orderBy: \'type\'" ng-class="{ \'selected\': $ctrl.selectedHeroes[hero.id], \'disabled\' : !$ctrl.selectedHeroes[hero.id] && $ctrl.maxReached }"><a ng-click=$ctrl.selectHero(hero) href=javscript:void()><img ng-src={{hero.avatar}}><div class=hero-detail-wrapper><span class=hero-name>{{hero.name}}</span></div></a></li></ul>',
...
angular.module('overwatch-hero-picker').component('owHeroPicker', {
    template:'<main class=content><ow-hero-list selected-heroes=$ctrl.selectedHeroes></ow-hero-list><h2>Who do you want to counter? <small>(choose 1 to 6)</small></h2><div class=recommendation ng-show=$ctrl.heroesList.length><h2>You should pick:</h2><ul class="recommended clearfix"><li ng-repeat="hero in $ctrl.heroesList | filter: $ctrl.recommendedFilter"><img ng-src={{hero.avatar}}><div class=score>{{$ctrl.abs($ctrl.heroCounterScore[hero.id])}}</div></li></ul><h2>And Avoid picking:</h2><ul class="avoid clearfix"><li ng-repeat="hero in $ctrl.heroesList | filter: $ctrl.avoidFilter"><img ng-src={{hero.avatar}}><div class=score>{{$ctrl.heroCounterScore[hero.id]}}</div></li></ul></div></main>',
    controller: function controller($q, $scope, HeroesService, HeroPickerService) {
        ...
        $scope.$watchCollection('$ctrl.selectedHeroes', function (value) {
            ...
            if (value && Object.keys(value).length > 0) {
                ...
                _this.heroesList.length = 0;
            }
        });

        this.getRecommendedHero = function () {
            return $q.all({
                recommendedCounterScore: HeroPickerService.getRecommendedHero(_this.selectedHeroes),
                heroesList: HeroesService.getHeroes()
            }).then(function (r) {
                _this.heroCounterScore = r.recommendedCounterScore;
        ...
        this.recommendedFilter = function (hero) {
            return _this.heroCounterScore[hero.id] <= 0;
        };
...
angular.module('overwatch-hero-picker').service('HeroPickerService', function ($q, HeroesService) {
    ...
    this.getRecommendedHero = function (opposingHeroes) {
        return $q.all({
