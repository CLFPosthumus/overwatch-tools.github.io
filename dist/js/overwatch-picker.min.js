...
angular.module('overwatch-hero-picker').component('owHeroPicker', {
    template:'<main class=content><ow-hero-list class=opponents selected-heroes=$ctrl.selectedHeroes></ow-hero-list><ow-map-list selected-map=$ctrl.selectedMap></ow-map-list><div class=recommendation ng-show=$ctrl.heroesList.length><ul class="recommended clearfix"><li ng-repeat="hero in $ctrl.heroesList | filter: $ctrl.recommendedFilter | limitTo: $ctrl.heroLimit"><img ng-src="{{ hero.avatar }}"><ow-hero-picker-score final=$ctrl.heroFinalScore[hero.id] counter=$ctrl.heroCounterScore[hero.id] map=$ctrl.heroMapScore[hero.id]></ow-hero-picker-score></li></ul><button ng-click="$ctrl.heroLimit = null" ng-show="$ctrl.heroLimit !== null">View all ratings</button></div><button class=hamburger ng-click="asideOpen = !asideOpen">Player settings</button><aside ng-class="{open : asideOpen}"><ow-player-hero-rating heroes-rating=$ctrl.heroesRating></ow-player-hero-rating></aside></main>',
    controller: function controller($q, $scope, HeroesService, HeroPickerService) {
        ...
        this.recommendedFilter = function (hero) {
            return _this.heroLimit != minHeroShown || _this.heroFinalScore[hero.id] >= 0;
        };
...
angular.module('overwatch-hero-picker').component('owPlayerHeroRating', {
    ...
    template:'<ul class=list><li ng-repeat="hero in $ctrl.heroes | orderBy: \'name\'"><img ng-src={{hero.avatar}}><div class=detail-wrapper><span class=name>{{hero.name}} <span ng-if=$ctrl.heroesRating[hero.id]>( {{$ctrl.heroesRating[hero.id]}} )</span></span></div><input type=range min=0 max=100 ng-model=$ctrl.heroesRating></li></ul>',
