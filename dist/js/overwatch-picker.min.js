...
angular.module('overwatch-hero-picker').component('owHelpBoxRemember', {
    template:'<label for=remember-help-close><input type=checkbox ng-checked=$ctrl.isKeepShowingChecked id=remember-help-close checked ng-click=$ctrl.rememberHelpClose($event)> Keep showing this box upon my next visit</label>',
...
angular.module('overwatch-hero-picker').component('owPlayerHeroRating', {
    ...
    template:'<ul class=list><li ng-repeat="hero in $ctrl.heroes | orderBy: \'name\'"><img ng-src={{hero.avatar}}><div class=detail-wrapper><span class=name>{{hero.name}} <span ng-if=$ctrl.heroesRating[hero.id]>( {{$ctrl.heroesRating[hero.id]}} )</span></span> <input type=range min=-5 max=5 ng-model=$ctrl.heroesRating[hero.id]></div></li></ul>',
